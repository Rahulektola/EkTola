 ...
 */
export function checkAdminAuth(authService) {
    if (!authService.isAuthenticated()) {
        window.location.href = '/admin-login.html';
        return false;
    }
    const decoded = authService.decodeToken(authService.accessToken);
    if (!(decoded === null || decoded === void 0 ? void 0 : decoded.is_admin)) {
        authService.logout();
        window.location.href = '/admin-login.html';
        return false;
    }
...
export class ConfirmationModal {
    constructor(title, message, confirmText = 'Confirm', cancelText = 'Cancel') {
        this.onConfirm = () => { };
        this.onCancel = () => { };
        // Create overlay
...
export class PaginationControls {
    constructor(containerId, initialPage, pageSize, total, onPageChange) {
...
export class AdminNavigation {
    constructor(containerId, activePage) {
        this.container = getElement(containerId);
 ...
 */
export async function apiRequest(endpoint, authService, options = {}) {
 ...
 */
export async function apiUpload(endpoint, authService, formData) {
    const url = `${baseURL}${endpoint}`;
