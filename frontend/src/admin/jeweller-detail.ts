...
interface ContactDiagnostics {
    ...
    jeweller_id: number;
    diagnostics: {
        total_contacts_all: number;
        total_contacts_active: number;
        total_contacts_deleted: number;
...
async function loadSegmentDistribution(): Promise<void> {
    try {
        ...
        const data = await apiRequest<ContactDiagnostics>(
...
async function loadContacts(): Promise<void> {
    ...
    try {
        ...
        const data = await apiRequest<ContactsResponse>(
            url,
            authService
        );
...
(window as Window & { showContactsDiagnostics?: typeof showContactsDiagnostics }).showContactsDiagnostics = showContactsDiagnostics;
